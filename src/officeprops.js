...
var typeConverters = {
    ...
    "enumDocSecurity": (e)=>e==0 ? "None" : 
                           e==1 ? "Document is password protected." :
                           e==2 ? "Document is recommended to be opened as read-only." :
                           e==4 ? "Document is enforced to be opened as read-only." :
                           e==8 ? "Document is locked for annotation." :
                                  "Unknown", //default
    "bool": (e)=>e=="false" ? "No" : e=="true" ? "Yes" : "Unknown",
    "ISO8601": (e)=> (e=/^P((\d+Y)?(\d+M)?(\d+W)?(\d+D)?)?(T(\d+H)?(\d+M)?(\d+S)?)?$/.exec(e)
                    .map(t=>Number(typeof t === "undefined" ? 0 : (t=t.replace(/[^\d]/g,""),t==""?0:t))),
                    e=Math.floor((e[2]*31104000+e[3]*2592000+e[4]*604800+e[5]*246060+e[7]*3600+e[8]*60+e[9])/60),
...
function getXmlFromZip(zip,fileName){
    return zip.file(fileName).async("text").then(function(text){
...
function translateMetaData(textObjects,names){
    ...
    textObjects.forEach((e,i,a)=>{
        if(e.path=="HeadingPairs/vt:vector/vt:variant/vt:lpstr"){
            headingPairsAndParts.push({"name": names.hasOwnProperty(e.value) ? names[e.value].name : e.value.replace(/ /g,""), 
                                       "length": a[i+1].value, 
                                       "value": []})
        }
    ...
    textObjects = textObjects.filter(e=>e.path!="HeadingPairs/vt:vector/vt:variant/vt:lpstr"
    ...
    textObjects.forEach(e=>{
        if(names.hasOwnProperty(e['path'])){
            ...
            createPropertyOrArray(editable,names[e['path']].name,{"value": e.value, "tvalue": translatedValue, "xmlProperty": e.path});
        }else{
            createPropertyOrArray(editable,e['path'],{"value": e.value, "tvalue": e.value, "xmlProperty": e.path});
        }
    });
...
function getTextFromNodelist(nodes,name,metaObjects){
    if(typeof metaObjects === 'undefined') {metaObjects=[]};
    ...
    Array.from(nodes).forEach(function(e){
        if(e.childNodes.length==1 && e.firstChild.nodeType===3){
            ...
            if(metaObject.path=="office:meta/meta:document-statistic" || metaObject.path === "office:meta/meta:template"){
...
function editXML(xml,metaData){
    for(key in metaData.editable){
        ...
        if(object.xmlProperty.includes("/@")){
            var nodes = xml.getElementsByTagName(object.xmlProperty.split("/").slice(-2,-1))
            for(var i=0; i<nodes.length; i++){
                nodes[i].getAttributeNode(object.xmlProperty.split("/").slice(-1)[0].replace("@","")).value=object.value;
            }
        }else{
        var nodes = xml.getElementsByTagName(object.xmlProperty.split("/").slice(-1));
            if(nodes.length > 0 && object.xmlProperty !=""){
                for(var i=0; i<nodes.length; i++){
                    var valueToInsert = object.value instanceof Array ? object.value[i < object.value.length ? i : object.value.length-1] : object.value;
...
function getModifiedMetaDataAsXml(officeFile,metaData){
    return loadFile(officeFile).then(function(zip){
        return getMetaDataAsXML(zip)
            .then(function(xmls){
...
OFFICEPROPS.removeMetaData = async function(officeFile,metaData){
    return loadFile(officeFile).then(function(zip){
        if(zip.OPformat === "office"){
            zip.remove("docProps/core.xml");
            zip.remove("docProps/app.xml");
    ...
    return loadFile(officeFile).then(function(zip){
        return getMetaDataAsXML(zip).then(function(files){
            return translateMetaData(
                    [].concat.apply([],files.map(file=>getTextObjectsFromXML(file))),properties);
        });
    }).catch(e=>{throw new Error(e)});
}
